...
import { Search, Filter, Download, LayoutGrid, BarChart3, LineChart } from 'lucide-react';

export default function PMSectionPage() {
    ...
    <div className="flex flex-col min-h-screen bg-[#F4F4F5] font-sans">
      <header className="bg-slate-900 text-white shrink-0 border-b border-slate-800">
        <div className="px-8 py-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1 uppercase tracking-widest font-bold">Analytics / {sectionTitle}</p>
          </div>
      ...
      <main className="flex-1 overflow-y-auto p-8 space-y-6">
        <div className="flex justify-between items-end">
          <div>
            <h2 className="text-2xl font-bold text-zinc-900 tracking-tight">{sectionTitle} Report</h2>
            <p className="text-[13px] text-zinc-500 mt-1 font-medium">Viewing real-time metrics for {sectionTitle.toLowerCase()}.</p>
          </div>
          <div className="flex items-center gap-3">
            <button className="bg-white border border-zinc-300 text-zinc-700 px-4 py-2 rounded text-[13px] font-bold flex items-center gap-2 hover:bg-zinc-50 shadow-sm transition-colors">
              <Filter className="w-4 h-4" /> Filter Timeframe
            </button>
            <button className="bg-indigo-600 text-white px-4 py-2 rounded text-[13px] font-bold shadow-sm hover:bg-indigo-700 transition-colors flex items-center gap-2">
        ...
        <div className="bg-white border border-zinc-300 shadow-sm flex flex-col min-h-[500px]">
          <div className="px-6 py-4 border-b border-zinc-200 flex justify-between items-center bg-zinc-50">
            <div className="relative w-96">
              ...
              <input 
                type="text" 
                placeholder={`Search \${sectionTitle.toLowerCase()} data...`}
            ...
            <div className="flex gap-2">
              <button className="p-1.5 border border-zinc-300 rounded text-zinc-500 bg-zinc-100"><LineChart className="w-4 h-4" /></button>
              <button className="p-1.5 border border-zinc-300 rounded text-zinc-500 bg-white"><LayoutGrid className="w-4 h-4" /></button>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 p-12">
            ...
            <p className="text-sm font-bold text-zinc-700 mb-1">Processing {sectionTitle} data...</p>
            <p className="text-[13px]">Insufficient data volume to render full analytics grid. Wait for more customer traffic.</p>
          </div>
        </div>
      </main>
    </div>
